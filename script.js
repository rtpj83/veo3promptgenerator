...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const openCharacterCreatorBtn = document.getElementById(
    'openCharacterCreatorBtn',
  );
  const characterCreatorModal = document.getElementById(
    'characterCreatorModal',
  );
  const closeCharacterCreatorBtn = document.getElementById(
    'closeCharacterCreatorBtn',
  );
  const loadCharacterModal = document.getElementById('loadCharacterModal');
  const closeLoadCharacterBtn = document.getElementById(
    'closeLoadCharacterBtn',
  );
  ...
  function describeSingleImage(type) {
    if (!singleUploadedImageData) {
      alert(
        "Silakan unggah gambar pada bagian 'Deskripsikan dari Gambar' terlebih dahulu.",
      );
      return;
    }
...
(
  ...
  function saveCharacter() {
    ...
    if (existingIndex > -1) {
      if (
        !confirm(
          `Karakter dengan nama "${characterName}" sudah ada. Apakah Anda ingin menimpanya?`,
        )
      ) {
        return;
      }
  ...
  describeSubjectBtn.addEventListener('click', () =>
    describeSingleImage('subject'),
  );
  describePlaceBtn.addEventListener('click', () =>
    describeSingleImage('place'),
  );
  ...
  closeCharacterCreatorBtn.addEventListener('click', () =>
    characterCreatorModal.classList.add('hidden'),
  );
  characterCreatorModal.addEventListener('click', (e) => {
    if (e.target === characterCreatorModal)
      characterCreatorModal.classList.add('hidden');
  });
  ...
  Object.keys(characterUploads).forEach((type) => {
    ...
    input.addEventListener('change', (e) =>
      handleCharacterImageUpload(e, type),
    );
